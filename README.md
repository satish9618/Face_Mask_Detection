...
![Scikit Learn](https://img.shields.io/badge/Scikit--Learn-Evaluation-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)
...
Model performance
The effect of different preprocessing and training strategies
...
DatasetSourceMain ContributionFace Mask 12KKaggleMask + No MaskMendeley Mask DatasetMendeley DataMaskHuman FacesKaggleNo MaskHandoverFace9 RGBVLM HandoverFace9No Mask
...
ClassLabelImages😷 Mask118,197🙂 No Mask017,513Total35,710
...
ClassImagesNo Mask1,752Mask1,819Total3,571
...
Pythontf.keras.applications.resnet50.preprocess_input()
...
ExperimentConfigurationE1Custom CNNE2Custom CNN + AugmentationE3Custom CNN + RegularizationE4Custom CNN + Augmentation + Regularization
...
ExperimentConfigurationAccuracyPrecisionRecallF1-ScoreROC-AUCE1Custom CNN97.76%99.77%95.82%97.76%99.89%E2CNN + Augmentation99.24%98.75%99.78%99.26%99.98%E3CNN + Regularization99.41%99.83%99.01%99.42%99.97%E4CNN + Augmentation + Regularization99.02%99.78%98.30%99.03%99.94%
...
The custom CNN was already highly capable (97.76% without transfer learning).
Augmentation had a particularly strong effect on recall.
Regularization produced the strongest recorded custom-CNN configuration.
The combined configuration did not automatically improve performance.
...
ModelAccuracyPrecisionRecallF1-ScoreROC-AUCResNet5099.89%99.84%99.95%99.89%100.00%MobileNetV299.83%99.89%99.78%99.83%100.00%EfficientNetB099.75%99.94%99.56%99.75%100.00%DenseNet12199.83%99.89%99.78%99.83%100.00%InceptionV399.89%99.78%100.00%99.89%100.00%
...
ExperimentModelAugmentationRegularizationPurposeE5ResNet50❌❌Transfer-learning baselineE6ResNet50✅❌Effect of augmentationE7ResNet50❌✅Effect of regularizationE8ResNet50✅✅Combined configuration
...
ParameterValueInput Size224 × 224 × 3Batch Size32OptimizerAdamLossBinary Cross-EntropyOutputSigmoidEpochs5Pretrained ModelResNet50Pretrained WeightsImageNetBackboneInitially Frozen
...
The custom CNN was already strong (97.76% baseline).
Augmentation improved recall dramatically (95.82% → 99.78%).
Regularization produced the strongest custom-CNN configuration (E3).
Combining techniques did not automatically improve performance.
Preprocessing had a major effect on pretrained models.
All five pretrained models performed very closely after preprocessing correction.
ResNet50 was selected for detailed transfer-learning experiments.
Accuracy alone is not enough — multiple metrics are necessary.


🔄 Complete Project Evolution

Dataset Expansion — Combined multiple public sources → 35,710 images
MediaPipe / BlazeFace — Face-detection stage introduced
MediaPipe Removal — Simplified to direct image classification
Custom CNN Experiments — E1 → E2 → E3 → E4
Pretrained Model Screening — 5 architectures compared
Preprocessing Correction — Dramatic performance improvement
Single Architecture Selection — ResNet50 chosen
Training Standardization — 5 epochs, no early stopping
...
Keep the train, validation, and test partitions fixed.
Keep random seeds fixed where possible.
Do not augment validation data.
Do not augment test data.
Use architecture-specific preprocessing for pretrained models.
Keep test images completely separate from training.
Record the exact experiment configuration.
Record training time.
Save model weights and notebook outputs.
Keep external test images separate from the benchmark dataset.
Use the same image size of 224 × 224.
Use the same binary label mapping:
0 = No Mask 1 = Mask
...
Augmentation and regularization solve different problems.
More complexity does not guarantee better results.
Preprocessing is part of the model.
Compare before committing.
Accuracy is not the whole story.
Very high benchmark performance requires careful interpretation.
External testing is useful as a sanity check.
The project is an experimental study, not just a classifier.
